...
export class ExpensesService {
    ...
    async findByRoomId(roomId: string) {
        ...
        const expenses = await this.expenseModel.find({ roomId: roomId }).populate('roomId');
        return expenses;
    }

    async update(id: string, updateExpenseDto: UpdateExpenseDto) {
        const updatedExpense = await this.expenseModel.findByIdAndUpdate(id, updateExpenseDto, { new: true }).populate('roomId');
        if (!updatedExpense) {
            throw new NotFoundException(`Expense with id ${id} not found`);
        }
